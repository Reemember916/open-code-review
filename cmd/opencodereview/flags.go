...
func (a *ocrFlagSet) StringListVarP(p *[]string, name, shorthand string, value []string, usage string) {
	suffix := ""
	if shorthand != "" {
		a.shortMap[shorthand] = name
		suffix = fmt.Sprintf(" (shorthand: -%s)", shorthand)
	...
	*p = append((*p)[:0], value...)
	a.fs.Var((*stringListFlag)(p), name, usage+suffix)
}
...
func (a *ocrFlagSet) StringListVar(p *[]string, name string, value []string, usage string) {
	*p = append((*p)[:0], value...)
	a.fs.Var((*stringListFlag)(p), name, usage)
}
...
func parseReviewFlags(args []string) (reviewOptions, error) {
	...
	a.BoolVar(&opts.staticAnalysis, "static-analysis", false, "run static analyzers and inject diff-related findings into the LLM prompt")
	a.StringVar(&opts.analyzers, "analyzers", "cppcheck", "comma-separated static analyzers to run when --static-analysis is enabled")
	a.StringVar(&opts.semgrepConfig, "semgrep-config", "", "semgrep config path or registry id used by the semgrep analyzer")
	a.IntVar(&opts.staticContext, "static-context", 3, "line distance from changed lines for keeping static-analysis findings")
	...
	if opts.staticContext < 0 {
		return opts, fmt.Errorf("--static-context must be a non-negative integer")
	}
...
Flags:
  ...
  --analyzers string      comma-separated static analyzers to run when --static-analysis is enabled (default "cppcheck")
  ...
  --semgrep-config string semgrep config path or registry id used by the semgrep analyzer
  --static-analysis       run static analyzers and inject diff-related findings into the LLM prompt
  --static-context int    line distance from changed lines for keeping static-analysis findings (default 3)
