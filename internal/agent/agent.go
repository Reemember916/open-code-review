...
type Args struct {
	...
	// StaticAnalyzers optionally run deterministic static-analysis tools before
	// LLM review. Findings are filtered to the current diff and injected into
	// the per-file prompt.
	StaticAnalyzers        []staticanalysis.Analyzer
	StaticDiffContextLines int
...
type Agent struct {
	...
	staticFindings    map[string][]staticanalysis.Finding
}
...
func (a *Agent) runStaticAnalysis() {
	if len(a.args.StaticAnalyzers) == 0 {
		return
	}
	files := make([]string, 0, len(a.diffs))
	for _, d := range a.diffs {
		if d.IsDeleted || d.IsBinary || d.NewPath == "" || d.NewPath == "/dev/null" {
			continue
		}
	...
	findings, warnings := staticanalysis.Run(staticanalysis.AnalyzeRequest{
		RepoDir:          a.args.RepoDir,
		Files:            files,
		Diffs:            a.diffs,
		DiffContextLines: a.args.StaticDiffContextLines,
	}, a.args.StaticAnalyzers)
	for _, warning := range warnings {
		a.recordWarning("static_analysis", "", warning)
	}
	if len(findings) == 0 {
		return
	}
	a.staticFindings = make(map[string][]staticanalysis.Finding)
	for _, finding := range findings {
		a.staticFindings[finding.Path] = append(a.staticFindings[finding.Path], finding)
	}
}
...
func (a *Agent) executeSubtask(ctx context.Context, d model.Diff) error {
	...
	newPath := d.NewPath
	staticFindings := "Static analysis was not enabled."
	if len(a.args.StaticAnalyzers) > 0 {
		staticFindings = staticanalysis.RenderForPrompt(a.staticFindings[newPath])
	}
	...
	for _, m := range rawMsgs {
		...
		content = strings.ReplaceAll(content, "{{static_analysis_findings}}", staticFindings)
		if planResult == "" {
			content = strings.ReplaceAll(content, "### 审查计划\n{{plan_guidance}}\n\n", "")
			content = strings.ReplaceAll(content, "### Review Plan (Optional)\n{{plan_guidance}}\n\n", "")
		} else {
